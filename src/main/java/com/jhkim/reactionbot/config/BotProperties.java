...
public class BotProperties {
    ...
    public static class ScreenTranslate {
        ...
        // 픽셀 단위 미세 변화 감지 임계 (타자기 연출 감지). 직전 프레임과 128x72 그레이스케일 비교에서
        // 변한 픽셀 수가 이 값을 초과하면 "아직 텍스트 출력 중"으로 보고 안정 대기.
        // dHash(8x9)는 글자 몇 개 추가 정도의 변화를 못 감지해 대사 앞부분만 번역되는 문제를 보완.
        // 대화창의 "다음" 깜빡이 커서(▼)는 2~4픽셀 수준이라 그보다 크게 잡아야 무한 대기 안 함.
        // 0 = 비활성 (dHash 만 사용).
        private int pixelDiffMinChanged = 8;
